...
export TOTAL_TRAINING_STEP=1000
...
TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=0 python src/train_more.py \
    --is_few True \
    --few $FEW \
    --resume_checkpoint "res/more[1]2_28original" \
    ...
    --output_dir "./res/more["$FEW"]"$OUT_DIR"continue" \
    --logging_dir "./res/more["$FEW"]"$OUT_DIR"continue/log" \
    --use_image True \
    --image_num $NUMBER \
    --use_text True \
    --text_num $NUMBER \
