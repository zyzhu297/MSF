...
class Blip2Qformer(Blip2Base):
    ...
    def __init__(
        ...
        self.tokenizer = self.init_tokenizer(model_path=former_path)

        self.visual_encoder, self.ln_vision = self.init_vision_encoder(
            vit_model, img_size, drop_path_rate, use_grad_checkpoint, vit_precision, model_path=vision_path
        )
        if freeze_vit:
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(
            num_query_token, self.visual_encoder.num_features, cross_attention_freq, model_path=former_path
        )
    ...
    def get_image_processor(self):
        return BlipImageEvalProcessor(image_size=self.img_size)

    def get_text_processor(self):
        return BlipCaptionProcessor()

    @classmethod
    def init_tokenizer(cls, truncation_side="right", model_path=None):
        if model_path is None:
            model_path = "bert-base-uncased"
        tokenizer = BertTokenizer.from_pretrained(model_path, truncation_side=truncation_side)
    ...
    @classmethod
    def init_Qformer(cls, num_query_token, vision_width, cross_attention_freq=2, model_path=None):
        if model_path is None:
            model_path = "bert-base-uncased"
...
def test(image_path, caption):
    ...
    text_input = txt_processors(caption)

    sample = {"image": image, "text_input": [text_input]}
    features_image = model.extract_features(sample, mode="image").image_embeds
    features_text = model.extract_features(sample, mode="text").text_embeds

    print(features_image.shape)
    print(features_text.shape)
    print(features_text[0,0])

if __name__ == "__main__":
    test(
        "/tmp_image/bike.jpeg",
        "a large fountain spewing water into haha the air"
    )