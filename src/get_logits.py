...
def main():
    ...
    for split in ['train']:
        ...
        for datas in tqdm(dataloder):
            ...
            # print(score)
            # print([h[0] for h in min_heap])

            if len(locals()[f"{ans}_heap"]) < length:
                heapq.heappush(locals()[f"{ans}_heap"], (score, ans, label, data_id))
            else:
                if score > locals()[f"{ans}_heap"][0][0]:
                    try:
                        heapq.heapreplace(locals()[f"{ans}_heap"], (score, ans, label, data_id))
                    except:
                        print((score, ans, label, data_id), data)
                        input()
        res = []
        for sentiment in ['positive', 'neutral', 'negative']:
            res.extend(locals()[f"{sentiment}_heap"])
        res.sort(key=lambda x: x[0], reverse=True)
        t = 0
        for i in res:
            t += i[1] == i[2]
        print('Accuracy', t / len(res))
        print('result', res)
        draw_confusion_matrix(lst)
        # 比如分成10个区间，区间是等宽的
        # bins = np.linspace(0.9999, 1, 100)  # 0到100分为10个区间
        bins = np.linspace(0.5, max(i[0] for i in lst), 50)  # 0到100分为10个区间
        draw_hist(lst, bins, T)
        # outcome = function2([i[0] for i in lst], 30, rbf_kernel)
        # print(outcome)
        ...
        #             if x == y:
        #                 t += 1
        #     print('Interval:', bins[i], bins[i+1], 'Accuracy:', t / cnt, 'Number:', cnt)
        for data in res:
            data_id = data[-1]
            cur_data = entire_data[data_id] 
            cur_data["label"] = data[1]
            few_data[data_id] = cur_data 
        with open(nxt_few_path, 'w') as f:
            json.dump(few_data, f, indent=4)


def draw_confusion_matrix(output):
    cm = confusion_matrix([y for _, (x, y) in output], [x for _, (x, y) in output])
    plt.clf()
    # 绘制热图
...
def draw_hist(lst, bins, T):
    # 绘制直方图
    plt.hist([i[0] for i in lst], bins=bins, edgecolor='black', alpha=0.7)
    ...
    plt.savefig("score_trainT={}.png".format(T), dpi=300, bbox_inches='tight')
...
def softmax_algorithm(logits):
    ...
    return num, max_senti


def function1(logits, T=0.03, t=-9):
    score = T * math.log(sum(math.e ** (lo / T) for lo in logits))
    return score

def function2(X, m, kernel, alpha=1):
    n = len(X)
    I = set()
    S = np.zeros(n)
    selected_samples = []
    ...
    mu = np.zeros(n)
    for i in range(n):
        mu[i] = np.sum(kernel(X[i], X[j]) for j in range(n)) / n
    ...
    for p in range(m):
        ...
        for i in range(n):
            if i not in I:
                value = S[i] - alpha * mu[i]
                if value < min_value:
                    min_value = value
                    i_star = i
        
        selected_samples.append(X[i_star])
        I.add(i_star)
        for i in range(n):
            if i not in I:
                S[i] = (1 - 1/(p+1)) * S[i] + (1/(p+1)) * kernel(X[i_star], X[i])
